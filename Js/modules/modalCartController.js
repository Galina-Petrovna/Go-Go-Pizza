    ...
    if (cartControl.cartData.length) {
        ...
        const cardsData = await Promise.all (
            cartControl.cartData.map(
                async item => await getData(
                    `https://go-go-pizza-api-5rt4.onrender.com/api/products/${item.id}`,
                ),
            ),
        );
...
const submitOrder = async (e) => {
    ...
    try {
        const response = await fetch('https://go-go-pizza-api-5rt4.onrender.com/api/orders/', {
            method: 'POST', 
            headers: {
